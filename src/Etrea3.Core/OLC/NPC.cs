            ...
            while (true)
            {
                ...
                switch (option)
                {
                    case 1:
                        ...
                        {
                            var itemToAdd = ItemManager.Instance.GetItem(itemID);
                            dynamic item = null;
                            switch(itemToAdd.ItemType)
                            {
                                case ItemType.Weapon:
                                    item = (Weapon)itemToAdd;
                                    break;

                                case ItemType.Armour:
                                    item = (Armour)itemToAdd;
                                    break;
                                ...
                                case ItemType.Consumable:
                                    item = (Consumable)itemToAdd;
                                    break;

                                case ItemType.Scroll:
                                    item = (Scroll)itemToAdd;
                                    break;

                                default:
                                    item = (InventoryItem)itemToAdd;
                                    break;
                            }
                            item.ItemID = Guid.NewGuid();
                            npc.Inventory.TryAdd(item.ItemID, item);
                        }
                        break;
