...
namespace Etrea3.Networking.API
{
    public class ShopController : ApiController
    {
        ...
        public IHttpActionResult Get(int id)
        {
            try
            {
                var shop = ShopManager.Instance.GetShop(id);
                if (shop == null)
                {
                    return NotFound();
                }
                return Ok(Helpers.SerialiseEtreaObject<Shop>(shop));
            }
            catch (Exception ex)
            {
                Game.LogMessage($"ERROR: Error in ShopController.Get(id): {ex.Message}", LogLevel.Error);
                return InternalServerError(ex);
            }
        }
