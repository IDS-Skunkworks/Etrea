...
namespace Etrea3.Networking.API
{
    public class SpellController : ApiController
    {
        ...
        {
            // Get a list of all Spells in the game
            try
            {
        ...
        [Route("api/spell/name/{name}")]
        public IHttpActionResult GetSpellByName(string name)
        {
            // Get a Spell by its name
            try
            {
                string decodeName = Uri.UnescapeDataString(name);
                var spell = SpellManager.Instance.GetSpell(decodeName);
                if (spell == null)
                {
                    return NotFound();
                }
                return Ok(Helpers.SerialiseEtreaObject<Spell>(spell));
            }
            catch (Exception ex)
            {
                Game.LogMessage($"ERROR: Error in SpellController.Get(name): {ex.Message}", LogLevel.Error);
                return InternalServerError(ex);
            }
        }

        [Route("{api/spell/id/{id:int}")]
        public IHttpActionResult GetSpellById(int id)
        {
            // Get a Spell by its ID number
            try
            {
                var spell = SpellManager.Instance.GetSpell(id);
                if (spell == null)
                {
                    return NotFound();
                }
            ...
            {
                Game.LogMessage($"ERROR: Error in SpellController.Get(id): {ex.Message}", LogLevel.Error);
                return InternalServerError(ex);
            }
        }
