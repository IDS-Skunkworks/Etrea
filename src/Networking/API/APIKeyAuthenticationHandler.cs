...
namespace Etrea3.Networking.API
{
    public class APIKeyAuthenticationHandler : DelegatingHandler
    {
        ...
        protected override Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
        {
            ...
            {
                Game.LogMessage($"WARN: Connection from {request.GetClientIPAddress()} did not provide an API key", LogLevel.Warning, true);
            ...
            {
                Game.LogMessage($"WARN: Connection from {request.GetClientIPAddress()} sent an invalid API key", LogLevel.Warning, true);
                return Task.FromResult(request.CreateResponse(System.Net.HttpStatusCode.Unauthorized, "Invalid API key"));
            }
            Game.LogMessage($"INFO: Connection from {request.GetClientIPAddress()} authenticated as Player {player}", LogLevel.Info, true);
