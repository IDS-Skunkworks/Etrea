...
namespace Kingdoms_of_Etrea.OLC
{
    internal static partial class OLC
    {
        ...
        private static void DeleteQuest(ref Descriptor desc)
        {
            ...
            {
                Quest q = null;
                if(uint.TryParse(input, out uint questID))
                {
                    q = QuestManager.Instance.GetQuest(questID);
                }
                else
                {
                    q = QuestManager.Instance.GetQuest(input);
                }
                if (q != null)
                {
                    if(DatabaseManager.DeleteQuest(ref desc, q.QuestID))
                    {
                        if (QuestManager.Instance.RemoveQuest(ref desc, q.QuestGUID))
                        {
        ...
        private static void EditExistingQuest(ref Descriptor desc)
        {
            ...
            desc.Send($"Enter ID of Quest to edit: ");
            var input = desc.Read().Trim(); 
        ...
        private static void CreateNewQuest(ref Descriptor desc)
        {
            ...
            while(!okToReturn)
            {
                ...
                sb.AppendLine();
                sb.AppendLine("Options:");
                sb.AppendLine($"1. Set Quest ID{Constants.TabStop}2. Set Quest Zone");
                sb.AppendLine($"3. Set Quest Name{Constants.TabStop}4. Set Quest Text");
                sb.AppendLine("5. Set Quest Type");
                sb.AppendLine($"6. Add Required Item{Constants.TabStop}7. Remove Required Item");
                        ...
                        switch(option)
                        {
                            ...
                            case 14:
                                if(ValidateQuestObject(ref desc, ref q, true))
                                {
                                    ...
                                    {
                                        if(QuestManager.Instance.AddNewQuest(ref desc, q))
                                        {
